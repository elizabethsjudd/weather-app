...
export const Heading = ({ attrs = {}, level = defaults.level, kind = defaults.kind, size = defaults.size, children }: HeadingConfig): JSX.Element => {
	...
	const attributes = combineAttributes(attrs, {
		...
		className: `${styles.heading} ${styles[`heading--${kind}--${size}`]} ${styles[`heading--${kind}`]}`,
	});
